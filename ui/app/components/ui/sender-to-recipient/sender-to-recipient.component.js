...
import { getOriginOfCurrentTab, getPermittedAccountsByOrigin, getSelectedAddress } from '../../../selectors'
...
function SenderAddress ({ addressOnly, checksummedSenderAddress, senderName, senderAddressCopied, setSenderAddressCopied }) {
...
export default function SenderToRecipient ({
  ...
  const permittedAccounts = permittedAccountsByOrigin[originOfCurrentTab]

  let showWarning = false
  if (permittedAccounts?.length > 0 && permittedAccounts.includes(senderAddress) && !permittedAccounts.includes(selectedAddress)) {
    showWarning = true
  }
  ...
  return (
    <div className={classnames('sender-to-recipient', variantHash[variant])}>
      ...
      >
        ...
        <SenderAddress addressOnly={addressOnly} senderName={senderName} senderAddressCopied={senderAddressCopied} setSenderAddressCopied={setSenderAddressCopied} />
        {showWarning && (
          <Tooltip
            position="bottom"
            ...
            containerClassName="sender-to-recipient__tooltip-container"
          >
            <div className="sender-to-recipient__tooltip-container--icon"><InfoIcon severity="warning" /></div>
          </Tooltip>
        )}
      </div>
